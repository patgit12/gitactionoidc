...
############ Role to be assumed with a federated pricipal  #############
data "aws_iam_policy_document" "git_aws_oidc" {
  statement {
    ...
    condition {
      ...
      values = ["repo:patgit12/*:*"] # first * is your repo and second * is your branch. Make sure to write it this way or you'll get the error: Error: Could not assume role with OIDC: Not authorized to perform sts:AssumeRoleWithWebIdentity  ### make sure to run terraform apply --auto-approve  after every change to this file ####
    }
